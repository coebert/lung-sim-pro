...
function updateVitals(
  ...
  let targetEtCO2 = 38;
  if (ventRatio < 0.7) {
    targetEtCO2 = 38 + (0.7 - ventRatio) * 60; // Hypercapnia
  } else if (ventRatio > 1.5) {
    targetEtCO2 = 38 - (ventRatio - 1.5) * 20; // Hypocapnia
    targetEtCO2 = Math.max(15, targetEtCO2);
  }

  // HR response to distress
  const distress = Math.abs(1 - oxygenScore) + Math.abs(1 - ventRatio);
  const targetHR = patient.baseHR + distress * 25;

  // BP response - high PEEP reduces BP
  const peepEffect = Math.max(0, settings.peep - patient.optimalPEEP) * 2;
  const targetSBP = patient.baseSBP - peepEffect + distress * 10;
  const targetDBP = patient.baseDBP - peepEffect * 0.5 + distress * 5;
